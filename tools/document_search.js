...
export async function createVectorStoreFromDocs({ docs }) {
  ...
  console.log("embeddings_class:", VectorStore?.embeddings?.constructor?.name);
...
export async function loadVectorStoreFromCache({
  ...
  try {
    ...
    const result = await PersistenceStore.loadVectorStore(
      embeddingProvider,
      embeddingApiKey
    );
...
export async function saveVectorStoreToCache({
  ...
  try {
    ...
    const success = await PersistenceStore.saveVectorStore(
      vectorStore,
      sourceFiles
    );
...
export function buildDocumentSearchTool({
  ...
  if (vectorStore) {
    ...
    AllDocs = [];
    console.log(
      "📋 VectorStore ayarlandı, AllDocs lazy loading ile yüklenecek"
    );
  } else if (useCache) {
...
*/
        ...
        if (!docs || docs.length === 0) {
          ...
          const { bm25Retriever } = await buildHybridRetriever({
            kVec,
            kLex,
            searchType,
          });
        ...
        let withScoresDocs = withScores
          .filter(([_, score]) =>
            typeof score === "number" ? score >= minScore : true
          )
          .map(([doc]) => doc);
        ...
        for (const d of docs) {
          const src = [
            d.metadata?.source,
            d.metadata?.page !== undefined && d.metadata?.page !== null
              ? `p.${d.metadata.page}`
              : null,
          ]
            .filter(Boolean)
